...
export const api = {
  ...
  getShifts: (bu?: string, date?: string) => fetchJson<ShiftReadiness[]>(`/shifts?business_unit=${bu || ''}&date=${date || ''}`),
  getReadiness: (bu?: string, date?: string) => fetchJson<ShiftReadiness[]>(`/readiness?business_unit=${bu || ''}&date=${date || ''}`),
