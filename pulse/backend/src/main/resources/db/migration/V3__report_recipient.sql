...
-- snapshot at send time (id/name/email/role), stored as TEXT rather than
-- JSONB: it is a write-once audit snapshot that is never queried by field,
-- so the entity maps it as a plain String and Jackson does the parsing.
